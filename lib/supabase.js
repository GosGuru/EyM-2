...
// Creación del cliente Supabase con valores por defecto para evitar errores en build
const defaultUrl = 'https://placeholder.supabase.co';
const defaultKey = 'placeholder-anon-key';

export const supabase = SUPABASE_PROJECT_EXISTS 
...
export async function fetchCategorias() {
  try {
    // Durante el build o si Supabase no está configurado, usar mock data
    if (!SUPABASE_PROJECT_EXISTS || typeof window === 'undefined') {
      console.warn('🔧 Usando datos mock (build time o Supabase no configurado)')
...
export async function fetchProductosPorCategoria(slug) {
  try {
    // Durante el build o si Supabase no está configurado, usar mock data
    if (!SUPABASE_PROJECT_EXISTS || typeof window === 'undefined') {
      console.warn('🔧 Usando productos mock (build time o Supabase no configurado)')
